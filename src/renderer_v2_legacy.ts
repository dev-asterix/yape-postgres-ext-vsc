...
export const activate: ActivationFunction = context => {
  return {
    ...
    `;
      ...
      const updateTable = () => {
        ...
        columns.forEach((col: string) => {
          ...
          th.style.maxWidth = '400px'; // Match cell max-width
          ...
          // Add resize handle
          th.style.position = 'relative'; // Needed for absolute positioning of handle
          ...
          resizeHandle.addEventListener('mouseenter', () => {
            resizeHandle.style.borderRight = '2px solid var(--vscode-focusBorder)';
          });
          resizeHandle.addEventListener('mouseleave', () => {
            resizeHandle.style.borderRight = '';
          });
      ...
      // Store chart instance for cleanup (update when chart is created)
      const originalUpdateChart = updateChart;
      const wrappedUpdateChart = () => {
        originalUpdateChart();
        if (chartInstance) {
    ...
    disposeOutputItem(id) {
      // Clean up chart instances when cell is disposed
      // Note: We cannot directly access the element by id, but the WeakMap
      // will automatically clean up when elements are garbage collected
      console.log(`[Renderer] disposeOutputItem called for: ${id}`);
    }
  };
};
